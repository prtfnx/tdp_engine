...
def handle_clipboard_copy(context):
    ...
    try:
        ...
        if isinstance(selected_sprite, Sprite): 
            _copied_sprite_data = {
                'image_path': selected_sprite.texture_path.decode() if hasattr(selected_sprite, 'texture_path') and isinstance(selected_sprite.texture_path, bytes) else str(selected_sprite.texture_path) if hasattr(selected_sprite, 'texture_path') else None,
            ...
            _copied_sprite_data = {
                ...
                'current_frame': selected_sprite.current_frame if hasattr(selected_sprite, 'current_frame') else None,
                'last_frame_time': selected_sprite.last_frame_time if hasattr(selected_sprite, 'last_frame_time') else None,
                'sheet_texture': selected_sprite.sheet_texture if hasattr(selected_sprite, 'sheet_texture') else None,
                'atlas_path': selected_sprite.atlas_path if hasattr(selected_sprite, 'atlas_path') else None
            }
...
def paste_copied_sprite(context):
    ...
    if _copied_sprite_data.get('frame_rects') is None:
        ...
        result = context.Actions.create_animated_sprite(
            **_copied_sprite_data,
            table_id=table_id,
            sprite_id=new_sprite_id,
            position=position
...
def _get_cursor_position_in_table_coords(context):
    ...
    try:
        ...
        print(f"Cursor position in screen coords: ({cursor_x}, {cursor_y}), converted to table coords: ({table_x}, {table_y})")
